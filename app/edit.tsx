...
export default function EditTurntableScreen() {
  const router = useRouter();
  const { id } = useLocalSearchParams();
  const dispatch = useDispatch();
  const turntable = useSelector((state: RootState) =>
    state.turntable.list.find((t) => t.id === id)
  );
  ...
  useEffect(() => {
    if (turntable) {
      setName(turntable.name);
      setOptions(
        turntable.options.length > 0
          ? turntable.options.map((o, i) => ({
              ...o,
              color: i % 2 === 0 ? "#f87171" : "#86efac",
              weight: 1,
            }))
          : [genOption("#f87171"), genOption("#86efac")]
      );
    }
  }, [turntable]);

  const handleOptionChange = (oid: string, text: string) => {
    setOptions((opts) => opts.map((o) => (o.id === oid ? { ...o, text } : o)));
  };
  const handleRemoveOption = (oid: string) => {
    setOptions((opts) =>
      opts.length > 1 ? opts.filter((o) => o.id !== oid) : opts
    );
  };
